...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
	...
	private mydslLexer lexer;
	private mydslParser parser;

	public GenParser(InputStream in) {
		this.reader = new InputStreamReader(in);
	}

	public mydslLexer getLexer() {
		if (lexer == null) {
			try {
				this.lexer = new mydslLexer(new ANTLRReaderStream(reader));
			} catch (IOException e) {
				handleError(e);
			}
		}
	...
	public mydslParser getParser() {
		if (parser == null) {
			parser = new mydslParser(new CommonTokenStream(getLexer()));
		}
		return parser;
	}
	...
	protected void internalLink() {
		...
		try {
			...
			XtendFacade facade = XtendFacade.create(ctx, "org::example::dsl::Linking");
	...
	public Issues doCheck() {
		...
		try {
			...
			String[] checkFiles = new String[]{"org::example::dsl::GenChecks","org::example::dsl::Checks"};
