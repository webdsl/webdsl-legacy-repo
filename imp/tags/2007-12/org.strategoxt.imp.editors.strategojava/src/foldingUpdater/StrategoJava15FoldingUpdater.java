...
import org.strategoxt.imp.runtime.services.FoldingHelper;
 ...
 *
 */
public class StrategoJava15FoldingUpdater extends FolderBase {
	...
	*/
	...
	private void makeAdjunctAnnotations(ASTNode theAST) {
		...
		for (int i = 0; i < adjuncts.size();) {
			...
			IToken previous_token = prsStream
					.getIToken(adjunct.getTokenIndex()), next_token = prsStream
					.getIToken(prsStream
							.getNext(previous_token.getTokenIndex())), comments[] = previous_token
	...
	public void sendVisitorToAST(HashMap newAnnotations, List annotations,
			Object ast) {
