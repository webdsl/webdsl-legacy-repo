...
import webdsl.safari.parser.Ast.ASTNode;

// Static, wizard-generated Java file
 ...
 */
public class WebDSLParseController extends SimpleLPGParseController implements
		IParseController {
	private WebDSLParser parser;

	private WebDSLLexer lexer;
	...
	public IASTNodeLocator getNodeLocator() {
		return new WebDSLASTNodeLocator();
	...
	private void createLexerAndParser(IPath filePath) {
		try {
			lexer = new WebDSLLexer(filePath.toOSString()); // Create the lexer
			parser = new WebDSLParser(lexer.getLexStream() /*, project*/); // Create the parser
		} catch (IOException e) {
			throw new Error(e);
		}
	}
	...
	public Object parse(String contents, boolean scanOnly,
		...
		//        	System.err.println("WebDSLParseController.parse:  caught CoreException while deleting problem markers; continuing to parse regardless");
		//        }
