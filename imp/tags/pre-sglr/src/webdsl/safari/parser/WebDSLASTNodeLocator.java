...
import webdsl.safari.parser.Ast.AbstractVisitor;

public class WebDSLASTNodeLocator implements IASTNodeLocator {
	...
	private final class NodeVisitor extends AbstractVisitor {
		...
		public boolean preVisit(ASTNode element) {
			...
			//System.out.println("WebDSLNodeLocator.NodeVisitor.preVisit(ASTNode):  Examining " + element.getClass().getName() +
			...
			if (nodeStartOffset <= fStartOffset && nodeEndOffset >= fEndOffset) {
				//System.out.println("WebDSLNodeLocator.NodeVisitor.preVisit(ASTNode) SELECTED for offsets [" + fStartOffset + ".." + fEndOffset + "]");
				fNode[0] = element;
